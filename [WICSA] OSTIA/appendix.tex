...
\begin{align}
% \bigwedge_{
%i \in \{ \mathbf{S} \bigcup \mathbf{B}\}} 
%& ( \ta{i} \Rightarrow \lnot \e{i}) ) \label{eq:1a}\\
%%
 \bigwedge_{
i \in \mathbf{B} } 
&(\ta{i} \Rightarrow 
...
%%	\end{subarray}
%%}
%%&(\startid{i} \iff (\lnot \mathbf{Y}(\id{i}) \land \id{i}))\\
%%%
%
% 
\bigwedge_{
...
%%\end{subarray}
%%}
%%&(\f{j} \land  (\mathbf{Y}(\lnot \f{j})\lor \ori) \iff \cl{\f{j}} = 0)\\
%%%
%%%
%% \bigwedge_{
%%\begin{subarray}{c}
...
%%\end{subarray}
%%}
%%&( \f{j} \Rightarrow  \f{j} \U (\f{j} \land \cl{\f{j}} > \texttt{minTimeToReboot}) \land \lnot \p{j}
%%% \land \lnot \add{j} UNCOMMENT IF ADD IS DISABLED DURING FAILURE
%% \land x=0)\\
%
% 
 \bigwedge_{
...
%%%%\newpage
%%%%
%%%%\textbf{Taking into account rates}:
%%%%\begin{itemize}
%%%%\item[] $\tar{i}{r}$ indicates that the \texttt{take()}/\texttt{nextTuple()} step is performed by the $i^{th}$ bolt/spout with rate $r$ (i.e. $r$ tuples are taken);
%%%%\item[] $\pr{i}{r}$ corresponds to the \texttt{execute()}/\texttt{transform()} step performed by the $i^{th}$ bolt/spout with rate r; 
%%%%\item[]$\sigma_i$ represents the ratio between the number of input tuples taken as input by the bolt and the number of new output tuples that will be emitted. $\sigma_i$ is used to outline the behaviour of the main operations implemented by bolts (considering the input/output ratio): 
%%%%\begin{itemize}
%%%%\item $1 \leq \sigma$: function
%%%%\item $0 \leq \sigma \leq 1$: filtering
%%%%\item $\sigma = 0, d > 0$: aggregation
%%%%\end{itemize}
%%%%\item[] $\er{i}{\sigma_i r}$ corresponds to the \texttt{emit()} step executed by the bolt/spout with rate $\sigma_i r$ (i.e. $\sigma_i r$ tuples are emitted).
%%%%
%%%%\end{itemize}
%%%%
%%%%
...
%%%%\end{subarray} 
%%%%}
%%%%&(\tar{i}{r} \Rightarrow \pr{i}{r} \wedge \mathbf{X}(\lnot 
%%%%\tar{i}{r} \,
%%%%\U \, \er{i}{\sigma_i r}
%%%%)) \label{eq:2b} \\
%%%%%
...
%%%%r \in R_T
%%%%\end{subarray}}
%%%%&\pr{i}{r} \Rightarrow \pr{i}{r} \Snc (\ori \lor \tar{i}{r}) 
%%%%\land \pr{i}{r} \, \U \, \er{i}{\sigma_i r} \land \lnot \f{i}) \label{eq:3b} \\
%%%%%
%%%% \bigwedge_{
%%%%\begin{subarray}{c}
%%%%i \in \mathbf{B}\\
%%%%r \in R_T
%%%%\end{subarray}}
%%%%%%&(E_i^{\sigma_i r} \Rightarrow P_i^r \wedge 
%%%%%%\lnot (
%%%%%%\bigvee_{c \in R_T}
%%%%%%T_i^{c}) 
%%%%%%\wedge 
%%%%%%\mathbf{X}(\lnot (
%%%%%%\bigvee_{c \in R_T}
%%%%%%E_i^{\sigma_i c}) ) \label{eq:4b} \\
%%%%%MB: aggiunto since
%%%%&(\er{i}{\sigma_i r} \Rightarrow \pr{i}{r} \wedge 
%%%%(\ori \lor \Y(\lnot 
%%%%\er{i}{\sigma_i c} \, \Snc \, \tar{i}{r})) \label{eq:4b} \\
%%%%%
%%%%%
%%%% \bigwedge_{
%%%%\begin{subarray}{c}
...
%} &\left( (\aX q_j = q_j) 
%\Rightarrow 
%(\add{j} \wedge \ta{j}{}) \vee (\lnot \add{j} \wedge \lnot \ta{j}{})
%\right)\\
\end{align*}
%
...
%
\end{align*}
Each bolt has a maximum take rate $\rth{j}$ that limits the number of tuples that can be taken from the receive queue at any moment.
