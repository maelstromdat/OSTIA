...
\author{
\IEEEauthorblockN{Marcello M. Bersani, Francesco Marconi and Damian A. Tamburri}
...
example, Twitter uses stream processing frameworks like Storm to analyse and
learn trending topics from billions of tweets per minute. However, maintaining
the quality of such architectures often requires iterative experiments on expensive multi-node clusters to continuously refactor the underlying architecture according to performance data. As an aid to designers and developers evolving their big data architectures, we
developed OSTIA (On-the-fly Static Topology Inference Analysis) that allows reverse-engineering of topologies for the purpose of: (a) visualising and refactoring elicited models while maintaining constraints that would only be evaluated at deployment and run-time; (b) tackling the occurrence of common anti-patterns across considered topologies; (c) exploiting existing verification techniques on elicited models. This paper illustrates OSTIA and evaluates its uses/benefits on three industrial-scale case studies.
%=======
%Big data architectures have been gaining momentum in the last years. For
%example, Twitter uses stream processing frameworks like Apache Storm to learn trending topics from billions of tweets per minute. However, maintaining applications' quality (or better, \emph{topologies} quality in Storm jargon) often requires iterative experiments on expensive multi-node clusters. As an aid to designers of streaming applications we
%developed OSTIA, that is, ``On-the-fly Static Topology Inference
%Analysis". OSTIA allows, in a DevOps fashion
%(i.e., through continuous feedback between development and operation), the reverse-engineering of application topologies for:
%(a) using previously existing verification\&validation techniques on elicited
%models; (b) visualising and refactoring elicited models while maintaining
%constraints that would only be evaluated at deployment and run-time; (c)
%tackling the occurrence of common anti-patterns across considered topologies. We
%illustrate the uses and benefits of OSTIA on three real-life industrial case
%studies.
%>>>>>>> Stashed changes
%=======
