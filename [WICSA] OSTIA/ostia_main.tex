...
the quality of developed applications (\emph{topologies} in Storm jargon) often
requires iterative experiments on expensive multi-node clusters. As an aid to
designers and developers evaluating their applications in a DevOps fashion
(i.e., through continuous feedback between development and operation), we
developed OSTIA, that is, ``On-the-fly Static Topology Inference
Analysis". OSTIA allows reverse-engineering of topologies for the purpose of:
...
constraints that would only be evaluated at deployment and run-time; (c) tackle
the occurrence of common anti-patterns across considered topologies. We
...
\section{Evaluation}
\input{eval}\label{eval}

\section{Discussion}
% no \IEEEPARstart
\input{disc}\label{disc}
